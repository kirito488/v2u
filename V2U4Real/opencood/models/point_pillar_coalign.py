...
class PointPillarCoAlign(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        for feat in feature_list:
            split_f = torch.tensor_split(feat, cum_sum_len[:-1].cpu())
            ego_feature_list.append(torch.stack([xx[0] for xx in split_f], dim=0))
        z_ego = self.backbone.decode_multiscale_feature(ego_feature_list)

        # downsample feature to reduce memory
        if self.shrink_flag:
            fused_feature = self.shrink_conv(fused_feature)
            z_ego = self.shrink_conv(z_ego)
        ...
        output_dict = {
            'psm': psm,
            'rm': rm,
            'Z_fused': fused_feature,
            'Z_ego': z_ego,
        }
        self._defense_z = {
            'Z_fused': fused_feature.detach(),
            'Z_ego': z_ego.detach(),
        }
