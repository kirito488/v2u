...
class AttBEVBackbone(nn.Module):
    ...
    def _ego_agent_features(self, x, record_len):
        """Take the ego (first) CAV from each record_len group. x: (sum_cav, C, H, W)."""
        cum_sum_len = torch.cumsum(record_len, dim=0)
        split_x = torch.tensor_split(x, cum_sum_len[:-1].cpu())
        return torch.stack([xx[0] for xx in split_x], dim=0)

    def _decode_ups(self, ups):
        if len(ups) > 1:
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(self.blocks)):
            ...
            x_ego = self._ego_agent_features(x, record_len)
            ...
            if len(self.deblocks) > 0:
                ...
                ego_ups.append(self.deblocks[i](x_ego))
            else:
                ups.append(x_fuse)
                ego_ups.append(x_ego)

        x = self._decode_ups(ups)
        x_ego = self._decode_ups(ego_ups)

        data_dict['spatial_features_2d'] = x
        data_dict['spatial_features_2d_ego'] = x_ego
        return data_dict
