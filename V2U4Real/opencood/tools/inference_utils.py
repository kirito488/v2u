...
def inference_late_fusion(batch_data, model, dataset):
    ...
    pred_box_tensor, pred_score, gt_box_tensor, gt_ids = \
...
def save_prediction_gt(pred_tensor, score_tensor, gt_tensor, gt_id_tensor, pcd, timestamp, save_path):
    """
    Save prediction and gt tensor to txt file.
    """
    pred_np = torch_tensor_to_numpy(pred_tensor)
    socre_np = torch_tensor_to_numpy(score_tensor)
    gt_np = torch_tensor_to_numpy(gt_tensor)
    gt_id_np = torch_tensor_to_numpy(gt_id_tensor)
    ...
    np.save(os.path.join(save_path, '%04d_score.npy' % timestamp), socre_np)
    np.save(os.path.join(save_path, '%04d_gt_id.npy' % timestamp), gt_id_np)
