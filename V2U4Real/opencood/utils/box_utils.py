...
def get_mask_for_boxes_within_range_torch(boxes, limit_range):
    """
    ...
    dim = boxes.shape[-1]
    if dim not in (2, 3):
        raise ValueError(f"Unsupported box shape {boxes.shape}; expected "
                         "last dimension 2 or 3.")

    lower = limit_range[:dim]
    upper = limit_range[3:3 + dim]
    boundary_lower_range = \
        torch.as_tensor(lower, dtype=boxes.dtype, device=device).reshape(1, 1, -1)
    boundary_higher_range = \
        torch.as_tensor(upper, dtype=boxes.dtype, device=device).reshape(1, 1, -1)

    mask = torch.all(
        torch.all(boxes[:, :, :dim] >= boundary_lower_range,
                  dim=-1) & \
        torch.all(boxes[:, :, :dim] <= boundary_higher_range,
                  dim=-1), dim=-1)
...
def create_bbx(extent):
    ...
    bbx = np.array([[extent[0]/2, -extent[1]/2, -extent[2]/2],
