...
def compute_iou(box, boxes):
    ...
    """
    intersections = np.array([box.intersection(b).area for b in boxes],
                             dtype=np.float32)
    unions = np.array([box.union(b).area for b in boxes], dtype=np.float32)
    return np.divide(intersections, unions,
