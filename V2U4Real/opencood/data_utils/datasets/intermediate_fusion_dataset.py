...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        root_dir = self.params['root_dir'] if self.train else self.params['validate_dir']
        scenario_folders = sorted([os.path.join(root_dir, x)
                                for x in os.listdir(root_dir) if
                                os.path.isdir(os.path.join(root_dir, x))])
        scenario_name = os.path.basename(scenario_folders[scenario_index])
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            scenario_names.append(ego_dict.get('scenario_name', f'scenario_{i}'))
            timestamps.append(ego_dict.get('timestamp', f'timestamp_{i}'))
