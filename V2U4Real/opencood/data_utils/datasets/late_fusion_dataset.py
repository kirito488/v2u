...
def _pose_to_list(pose):
    """x1_to_x2 requires a Python list; a numpy (6,) pose hits linalg.inv on 1D."""
    if isinstance(pose, (list, tuple)):
        return list(pose)
    return np.asarray(pose).reshape(-1).tolist()
...
class LateFusionDataset(basedataset.BaseDataset):
    ...
    def __init__(self, params, visualize, train=True, isSim=False):
        ...
        # Optional: train/eval a single agent only ("1"=vehicle, "2"=UAV).
        # Point clouds are still projected to vehicle (ego) lidar frame.
        fusion_args = params.get('fusion', {}).get('args', None)
        self.only_cav_id = None
        if isinstance(fusion_args, dict):
            cid = fusion_args.get('only_cav_id', None)
            if cid is not None:
                self.only_cav_id = str(cid)
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        transformation_matrix = \
            x1_to_x2(_pose_to_list(selected_cav_base['params']['lidar_pose']),
                     _pose_to_list(ego_pose))
    ...
    def get_item_train(self, base_data_dict):
        ...
        # Fixed single agent (vehicle/UAV-only), else random CAV for late fusion.
        if self.only_cav_id is not None:
            assert self.only_cav_id in base_data_dict, \
                "only_cav_id={} not in frame keys {}".format(
                    self.only_cav_id, list(base_data_dict.keys()))
            selected_cav_id = self.only_cav_id
            selected_cav_base = base_data_dict[self.only_cav_id]
    ...
    def get_item_test(self, base_data_dict):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            if self.only_cav_id is not None and str(cav_id) != self.only_cav_id:
                continue
            # Single-agent-only was trained without COM_RANGE filtering.
            if self.only_cav_id is None:
                distance = \
            ...
            cav_lidar_pose = _pose_to_list(selected_cav_base['params']['lidar_pose'])
            transformation_matrix = x1_to_x2(cav_lidar_pose, _pose_to_list(ego_lidar_pose))
            selected_cav_processed = \
                self.get_item_single_car(selected_cav_base, ego_lidar_pose)
            selected_cav_processed.update({'transformation_matrix':
                                               transformation_matrix})
            # Single-agent-only: always expose as "ego" for late inference head.
            if self.only_cav_id is not None:
                update_cav = "ego"
            else:
