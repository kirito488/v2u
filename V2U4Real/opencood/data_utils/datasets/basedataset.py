...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True, isSim=False):
        ...
        self.lidar_type = params['lidar_type']
        ...
        self.scenario_names = {}
        func_map = {
            "1": self.load_ground_camera_files,
            "2": self.load_air_camera_files
        }
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            self.scenario_names[i] = scenario_name
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                # save ouster, ruby or m1 pcd files to the dictionary
                lidar_path = os.path.join(cav_path, self.lidar_type)

                # save img files to the dictionary
                camera_path = os.path.join(cav_path, 'camera')

                # save ouster, ruby and m1 yaml files to the  dictionary
                yaml_path = os.path.join(cav_path, 'yaml')
                yaml_path = os.path.join(yaml_path, self.lidar_type)

                # use the frame number as key, the full path as the values
                yaml_files = \
                    sorted([os.path.join(yaml_path, x)
                            for x in os.listdir(yaml_path) if
                ...
                for timestamp in timestamps:
                    ...
                    yaml_file = os.path.join(yaml_path,
                                             timestamp + '.yaml')
                    lidar_file = os.path.join(lidar_path,
                                              timestamp + '.pcd')
                    
                    func = func_map.get(str(cav_id))
                    if func is not None:
                        camera_files = func(camera_path, timestamp)
                    else:
                        raise ValueError("Unsupported cav_id")
    ...
    def get_scenario_name(self, idx):
            """
            Get the scenario name for a given index.
            ...
            -------
            scenario_name : str
                The name of the scenario (folder name).
            """
            scenario_index = 0
            for i, ele in enumerate(self.len_record):
                if idx < ele:
                    scenario_index = i
                    break
            return self.scenario_names.get(scenario_index, f"scenario_{scenario_index}")
    ...
    @staticmethod
    def load_ground_camera_files(camera_path, timestamp):
        """
        ...
        ----------
        camera_path : str
            The full file path of current cav's cameras.
        ...
        camera_left_path = os.path.join(camera_path, 'left')

        camera_middle_path = os.path.join(camera_path, 'middle')
         
        camera_right_path = os.path.join(camera_path, 'right') 

        camera_left_file = os.path.join(camera_left_path,
                                    timestamp + '.jpg')
        camera_middle_file = os.path.join(camera_middle_path,
                                    timestamp + '.jpg')
        camera_right_file = os.path.join(camera_right_path,
                                    timestamp + '.jpg')

        return [camera_left_file, camera_middle_file, camera_right_file]

    @staticmethod
    def load_air_camera_files(camera_path, timestamp):
        """
        Retrieve the paths to all camera files.
        ...
        ----------
        img_path : str
            The full file path of current cav's camera.
        ...
        camera_files : list
            The list containing all camera jpg file paths.
        """
        camera_file = os.path.join(camera_path,
                                    timestamp + '.jpg')
