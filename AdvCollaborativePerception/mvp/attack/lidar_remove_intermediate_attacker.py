...
class LidarRemoveIntermediateAttacker(Attacker):
    def __init__ (self, perception, dataset=None, step=100, sync=1, init=True, online=True):
        ...
        self.max_perturb = 3   # 降低攻击强度，减少 FP 泄漏
    ...
    def run(self, multi_frame_case, attack_opts):
        ...
        for frame_index, frame_id in enumerate(attack_opts["frame_ids"]):
            ...
            # Online planner may skip frames (None); do not crash on coast holes.
            if bbox_to_remove is None:
                clean = self.perception.run(case[frame_id], ego_id)
                case[frame_id][ego_id]["pred_bboxes"] = clean[0]
                case[frame_id][ego_id]["pred_scores"] = clean[1]
                info[frame_id][ego_id] = {
                    "pred_bboxes": clean[0], "pred_scores": clean[1]}
                init_perturbation = None
                continue
            ...
            if real_bbox_to_remove is None:
                real_bbox_to_remove = bbox_to_remove
            ...
            result = self.perception.attack_intermediate(optimize_case, ego_id, attacker_id, max_perturb=self.max_perturb, mode="remove", bbox=bbox_to_remove_ego, max_iteration=self.step, lr=self.learn_rate, real_case=real_case, original_case=original_case, real_original_case=real_original_case, real_bbox=real_bbox_to_remove_ego, init_perturbation=init_perturbation, feature_size=5)   # 收窄扰动窗口，降低攻击范围
