...
class LidarRemoveEarlyAttacker(Attacker):
    def __init__(self, dataset=None, advshape=1, dense=3, sync=1):
        ...
        # 3D 车模 / adv-shape：有文件就用，没有则 meshes=None，run_core_sample 走四面墙。
        self.default_car_model = "car_0200"
        self.default_car_mesh = None
        self.mesh = None
        self.meshes = None
        ply_path = os.path.join(model_3d_path, "{}.ply".format(self.default_car_model))
        perturb_path = os.path.join(model_3d_path, "remove", "mesh_perturb.npy")
        if os.path.isfile(ply_path):
            self.default_car_mesh = o3d.io.read_triangle_mesh(ply_path)
        if os.path.isfile(perturb_path):
            try:
                perturb = np.load(perturb_path)
                adv_attacker = AdvShapeAttacker()
                self.mesh = adv_attacker.perturb_mesh(adv_attacker.mesh, perturb)
                self.meshes = [
                    self.mesh.select_by_index(vertex_indices)
                    for vertex_indices in adv_attacker.mesh_divide]
            except Exception:
                self.advshape = 0
        elif self.advshape > 0:
            self.advshape = 0
    ...
    def run(self, multi_frame_case, attack_opts):
        ...
        for frame_id in frame_ids:
            ...
            bbox_to_remove = None
            if "bboxes" in attack_opts and attack_opts["bboxes"][frame_id] is not None:
                bbox_to_remove = attack_opts["bboxes"][frame_id]
                ...
                try:
                    ...
                    bbox_to_remove = None
            if bbox_to_remove is None:
                if frame_id in attack_opts["frame_ids"]:
                    attack_info.append({})
                    new_case[frame_id][attack_opts["attacker_vehicle_id"]] = copy.deepcopy(
                        single_vehicle_case)
                continue
            ...
            x, info = core_function(single_vehicle_case, {
                ...
                "object_id": attack_opts.get("object_id"),
                "bbox": bbox_to_remove,
    ...
    def run_core(self, single_vehicle_case, attack_opts):
        ...
        object_index = -1
        if attack_opts.get("bbox") is not None:
            ...
            try:
                ...
                bbox_to_remove = attack_opts.get("bbox")
    ...
    def run_core_sample(self, single_vehicle_case, attack_opts):
        ...
        for i in replace_indices:
            ...
            replace_data.append(
                replace_data_list[
                    np.random.choice(mesh_weight.shape[0], p=w / s)
                ][i]
            )
        replace_indices = replace_indices[:len(replace_data)]
        replace_data = np.array(replace_data) if replace_data else np.zeros((0, 3))
