...
class LidarSpoofIntermediateAttacker(Attacker):
    ...
    def run(self, multi_frame_case, attack_opts):
        ...
        for frame_index, frame_id in enumerate(attack_opts["frame_ids"]):
            ...
            result = self.perception.attack_intermediate(optimize_case, ego_id, attacker_id, max_perturb=self.max_perturb, mode="spoof", bbox=bbox_to_spoof_ego, max_iteration=self.step, lr=self.learn_rate, real_case=real_case, original_case=original_case, real_original_case=real_original_case, real_bbox=real_bbox_to_spoof_ego, init_perturbation=init_perturbation, feature_size=5)
