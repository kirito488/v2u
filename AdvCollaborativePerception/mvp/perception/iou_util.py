...
# CUDA sort_vertices 内核可选：装了(nvcc 编译过)就用它；否则用下面的纯 torch 等价实现。
try:
    from .cuda_op.cuda_ext import sort_v
    _HAS_CUDA_SORT = True
except Exception:
    _HAS_CUDA_SORT = False
    sort_v = None
...
def _sort_vertices_puretorch(vertices:torch.Tensor, mask:torch.Tensor, num_valid:torch.Tensor):
    """
    纯 torch 版 sort_v：把有效(相交)顶点按绕质心的角度排序，返回 (B, N, 9) 的长整型索引。
    结构 = [按角度升序的有效顶点..., 首顶点(闭合), 首顶点填充]。
    填充用首顶点：其与自身的叉积为 0，不改变 shoelace 面积（与 CUDA 版结果一致）。
    """
    B, N, M = mask.shape
    dev = vertices.device
    # 每个顶点的极角；无效顶点置 +inf 使 argsort 排到末尾
    ang = torch.atan2(vertices[..., 1], vertices[..., 0])
    ang = torch.where(mask, ang, torch.full_like(ang, float('inf')))
    sorted_idx = torch.argsort(ang, dim=-1, stable=True)        # (B, N, M) 有效顶点按角度升序在前
    K = num_valid.unsqueeze(-1)                                  # (B, N, 1)
    p = torch.arange(9, device=dev).view(1, 1, 9).expand(B, N, 9)
    first_col = sorted_idx[..., 0:1].expand(B, N, 9)
    idx = torch.where(p < K, torch.gather(sorted_idx, -1, p), first_col)
    return idx
...
def sort_indices(vertices:torch.Tensor, mask:torch.Tensor):
    ...
    return _sort_vertices_puretorch(vertices_normalized, mask, num_valid)
