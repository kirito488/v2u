...
def box3d_iou(corners1, corners2):
    ...
    # NOTE: `from numpy import *` shadows the builtin min/max with np.min/np.max,
    # so use np.minimum/np.maximum here (np.min(builtin1, builtin2) would interpret
    # the second argument as `axis` and crash).
    ymax = np.minimum(corners1[0,1], corners2[0,1])
    ymin = np.maximum(corners1[4,1], corners2[4,1])

    inter_vol = inter_area * np.maximum(0.0, ymax-ymin)
